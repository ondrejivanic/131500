...
require(knitr)
opts_knit$set(root.dir = '~/Projects/131500/')
opts_chunk$set(
  cache = TRUE, cache.path = "~/Projects/131500/target/cache/",
  message = FALSE, tidy = FALSE,
  fig.path = "~/Projects/131500/target/figures/",
  base.dir = "~/Projects/131500/target/"
)
...
values <- unique(c(seq(min(new.old.bins$count.diff), 0, length.out = 6), seq(mm, max(new.old.bins$count.diff), length.out = 6)))
mid = abs(min(new.old.bins$count.diff) / max(new.old.bins$count.diff))
rescaled <- unique(c(
  rescale(seq(min(zz$count.diff), 0, length.out = 6), c(0, mid)), 
  rescale(seq(0, max(zz$count.diff), length.out = 6), c(mid, 1))
))
# display.brewer.all()
...
l_ply(sort(unique(new.old.bins$time)), function(t, d, grad) {
  ...
  ggsave(filename = paste0(format(trunc(Sys.time(), "day") + t, "%H:%M"), ".png"), plot = g, width = 10)
