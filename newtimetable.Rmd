...
timetable.stats <- function(one.day) {
  list(
     "total.trips" = length(unique(one.day$trip_id)),
     "bus.trips"   = length(unique(one.day[one.day$route_type == "Bus", ]$trip_id)),
     "train.trips" = length(unique(one.day[one.day$route_type == "Train", ]$trip_id)),
     "ferry.trips" = length(unique(one.day[one.day$route_type == "Ferry", ]$trip_id))
  )
}

old.stats <- timetable.stats(old.singleday)
new.stats <- timetable.stats(new.singleday)
...
```{r, network-changes-animation, fig.height=10, fig.width=10, fig.show='animate', echo=FALSE}
